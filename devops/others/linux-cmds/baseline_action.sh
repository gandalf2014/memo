...
cat /etc/ssh/sshd_config | grep -q "^ClientAliveInterval" || echo "ClientAliveInterval 300" >> /etc/ssh/sshd_config; systemctl restart sshd
cat /etc/ssh/sshd_config | grep -q "^ClientAliveCountMax" || echo "ClientAliveCountMax 0" >> /etc/ssh/sshd_config; systemctl restart sshd
echo ----------------------------扫描特定的IP连接SSH-------------------------------------
